...
class StaffRegisterAPI(MethodView):
    ...
    def post(self):
        ...
        #if params for chief specialty=Chief and chief=True, else specialty = request.form['specialty']
        try:
            request.headers["chiefparams"]
            specialty="Investigator"
            specialty_description = "Administrator"
            verification_status = True 
            acceptance_status = True
            moduleaccesslist =[]
        except:
            specialty=request.json['specialty']
            specialty_description = request.json["specialty_description"]
        ...
        if existing_user:
            ...
            StaffProfilesEntry = StaffProfiles(
                ...
                timeOfRegistration=str(int(float(now()))) 
            )
            ...
            #     pubkey=items.pubkey
                ...
                if items.specialty=="Investigator":
                    pass
                else:
                    pass

            pubkey=None
            loginbody={"token":newtoken, "pubkey":pubkey, "homepage":homepage, "status":username+" logged in", "specialty" : specialty}
        ...
        if StaffProfiles.objects.filter(username=username):
            for items in StaffProfiles.objects.filter(username=username):
                ...
                if  hashed_password != tblpasswordhash:
                    ...
                    loginbody={"token":newtoken, "homepage":homepage, "specialty": specialty}
                    return  jsonify(loginbody), 200
        else:
            error = {
                "code": "USER_DOES_NOT_EXIST"
...
class StaffEmailForgotPasswordAPI(MethodView):
    ...
    def post(self):
        ...
        # if email != "today":
        if email in emails:
            # url="https://kencorpus.co.ke/staff/forgotpassword
            recoverparams=hash_text(email+str(int(float(now()))) + str(r(0,1000000000000000000000000)))
            RecoverPassword(recoverparams=recoverparams,email=email,timeOfRegistration=str(int(float(now())))).save()
            ...
            if names.recoverparams==recoverparams:
                if names.recoverparamsstatus==False:
                    ...
                    for identity in StaffProfiles.objects.filter(email=names.email):
                        ...
                        identity.update(passwordhash=passwordhash)
                        #to change passwordhash to list of tuples with timestamp and password as values. Append new tuple to list
                        #send email
                    message,code={"success":"password changed" },200
                    return jsonify(message),code
                    
                else:
                    message, code={"fail":"expired link"},403
                    return jsonify(message),code
            else:
                 message,code={"fail":"check recovery url"},403
                 return jsonify(message),code
...
class LogoutPageAPI(MethodView):
    ...
    def post(self):
        ...
        if request.headers["token"]:
            ...
            return jsonify({"error":"wrong device"}), 403
...
class StaffHomePageAPI(MethodView): 
    ...
    def post(self):
        #homepage params
        return "Success and Welcome to KenCorpus"
...
# # ADMIN REGISTER
# class ChiefRegisterAPI(MethodView):


#     # REGISTRATION ENDPOINT /admin/
#     #GET REACT HTML
#     def get(self):
#         Registerform= AdminRegistrationForm()
#         return render_template( 'accounts/adminregister.html',
#             form=Registerform)
...
#                 "USERNAME_OR_EMAIL_IS_MISSING"
#             }
#             return redirect (url_for("auth_app.adminregister_api", message="USERNAME OR EMAIL IS MISSING"))




#         existing_user = Admin_Table.objects.filter(username=username).first()
#         existing_email = Admin_Table.objects.filter(email=email).first()
...
#                 "code": "USERNAME_IS_TAKEN"
#             }
#            return redirect (url_for("auth_app.adminregister_api", message="USERNAME IS TAKEN"))

#         elif existing_email:
#             error = {
#                 "code": "EMAIL_IS_ALREADY_IN_USE"
#             }
#             return redirect (url_for("auth_app.adminregister_api", message="EMAIL IS ALREADY IN USE"))
